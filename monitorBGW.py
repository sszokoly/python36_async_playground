...
CONFIG = {
    ...
    "storage_maxlen": 900,
    "storage": None,
    "except_script_template": EXPECT_SCRIPT_TEMPLATE,
    "discovery_commands": [
        "set utilization cpu",
...
reRTPDetailed = re.compile(r''.join((
...
def create_bgw_script(bgw: BGW) -> List[str]:
    ...
    expect_script_template = unwrap_and_decompress(
        CONFIG['except_script_template']
    )
    
    expect_script = expect_script_template.format(**{
        "host": bgw.host,
    ...
    return ["/usr/bin/env", "expect", "-c", expect_script]
...
def create_query_gateway_tasks(
    queue=None,
...
def get_query_gateway_tasks() -> List[asyncio.Task]:
    """
    ...
    while True:
        try:
            ...
            async with semaphore:
                ...
                stdout = await exec_cmd(*script, timeout=timeout, name=bgw.host)
...
async def discover_gateways(storage=None, callback=None) -> Tuple[int, int]:
    ...
    ip_filter = CONFIG.get("ip_filter", [])
    ...
    tasks = create_query_gateway_tasks()
...
async def poll_gateways(storage = None, callback=None) -> None:
    ...
    tasks = create_query_gateway_tasks(queue=queue, discovered_only=False)
    task = create_task(
...
def must_resize(stdscr: "_curses._CursesWindow", miny: int, minx: int) -> bool:
    """
    Check if the screen needs to be resized.

    This function checks if the screen needs to be resized.
    It will return True if the screen needs to be resized and False otherwise.

    Args:
        stdscr: The curses window to check and display on.
        miny: The minimum number of rows required.
        minx: The minimum number of columns required.

    Returns:
        bool: True if the screen needs to be resized, False otherwise.
    """
    stdscr.erase()
    maxy, maxx = stdscr.getmaxyx()
    ...
    msgs = [
        f"Resize screen to at least {miny:>3} x {minx:>3}",
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Monitors Avaya Branch Gateways')
    parser.add_argument('-c', dest='color_scheme',
    ...
    parser.add_argument('-n', dest='lastn_secs',
                        default=CONFIG.get('lastn_secs', 30),
    ...
    parser.add_argument('-l', dest='loglevel',
                        default=CONFIG.get('loglevel', 'WARNING'),
                        help='loglevel, default WARNING')
    parser.add_argument('-t', dest='timeout',
                        default=CONFIG.get('timeout', 15),
                        help='query timeout, default 15s')
    parser.add_argument('-f', dest='polling_secs',
                        default=CONFIG.get('polling_secs', 5),
                        help='polling frequency, default 5s')
    parser.add_argument('-i', dest='ip_filter', metavar='IP', nargs='+',
                        default=CONFIG.get('ip_filter', []),
                        help='BGW IP(s) filter, default empty')
    parser.add_argument('-s', dest='storage_maxlen',
                        default=CONFIG.get('storage_maxlen', 900),
                        help='max number of RTP stats to store, default 900')
    ...
    args.storage = SlicableOrderedDict(maxlen=args.storage_maxlen)
